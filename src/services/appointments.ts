...
function normalize(a: any): Appointment {
  ...
  return {
    ...
    refundRequestedAt:   a.refund_requested_at  ?? undefined,
    refundProcessedAt:   a.refund_processed_at  ?? undefined,
  };
}
...
export async function requestRefund(id: string, amount: number): Promise<Appointment> {
  const res = await fetch(`${API}/appointments/${id}/refund/request`, {
    method:  "PATCH",
    headers: { "Content-Type": "application/json" },
...
// Admin confirms refund + triggers email to customer
export async function confirmRefund(id: string, amount: number): Promise<Appointment> {
  const res = await fetch(`${API}/appointments/${id}/refund/confirm`, {
    method:  "PATCH",
    headers: { "Content-Type": "application/json" },
    body:    JSON.stringify({ amount }),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.message ?? "Failed to confirm refund");
  }
  return normalize(await res.json());
}

// Customer submits their GCash or bank details
export async function submitRefundDetails(id: string, dto: {
  method:      string;
  account:     string;
  accountName: string;
  note?:       string;
}): Promise<Appointment> {
  const res = await fetch(`${API}/appointments/${id}/refund/details`, {
    method:  "PATCH",
    headers: { "Content-Type": "application/json" },
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.message ?? "Failed to submit refund details");
  }
  return normalize(await res.json());
}

// Admin marks refund as processed + logs to transactions
export async function processRefund(id: string): Promise<Appointment> {
  const res = await fetch(`${API}/appointments/${id}/refund/process`, {
    method: "PATCH",
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
...
// Admin rejects refund request + notifies customer
export async function rejectRefund(id: string, reason?: string): Promise<Appointment> {
  const res = await fetch(`${API}/appointments/${id}/refund/reject`, {
    method:  "PATCH",
    headers: { "Content-Type": "application/json" },
    body:    JSON.stringify({ reason }),
  });
