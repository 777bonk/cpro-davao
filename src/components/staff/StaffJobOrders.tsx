        ...
        <div className="p-6 space-y-3">
          ...
          <div className="pt-2">
            ...
            <div className="relative">
              <select className={inputClass + " appearance-none pr-8"} value={status}
...
export function StaffJobOrders({ onNavigate }: { onNavigate?: (s: string) => void }) {
  ...
  const pending   = jobs.filter(j => ["Pending","Confirmed","Scheduled"].includes(j.status)).length;
    ...
    <div className="space-y-6 w-full">
      ...
      <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="Search by service, customer, or vehicle..."
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="divide-y divide-white/5">
              {filtered.map(job => (
                <div key={String(job.id)} onClick={() => setViewJob(job)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-0.5">
                      ...
                      <span className="text-xs text-white/50 flex items-center gap-1">
                        <Car className="w-3 h-3" />{job.vehicle}
                      </span>
                      <span className="text-xs text-white/50 flex items-center gap-1">
                        <Calendar className="w-3 h-3 text-[#E41E6A]" />{formatDate(job.date)}
                      </span>
                    </div>
                  </div>
